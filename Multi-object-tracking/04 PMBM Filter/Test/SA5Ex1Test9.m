...
T = 1;
sigma = 0.1;
motion_model = motionmodel.cvmodel(T,sigma);
...
initial_state(1).x = [0; 0; 0; 0];       tbirth(1) = 1;   tdeath(1) = 50;
initial_state(2).x = [20; 20; -0; 0];    tbirth(2) = 20;  tdeath(2) = 70;
initial_state(3).x = [-20; 10; 0; 0];  tbirth(3) = 40;  tdeath(3) = 90;
initial_state(4).x = [-10; -10; 0; 0];   tbirth(4) = 60;  tdeath(4) = K;

birth_model = repmat(struct('w',log(0.03),'x',[],'P',diag([1 1 1 1].^2)),[1,4]);
birth_model(1).x = [0; 0; 0; 0];
birth_model(2).x = [20; 20; -0; 0];
birth_model(3).x = [-20; 10; 0; 0];
birth_model(4).x = [-10; -10; 0; 0];
...
for k = 1:length(objectdata.X)
    init = [initial_state.x];
%     measdata{k} = init(1:2, :)+[100;100];
    measdata{k} = [init(1:2, :) + 1*rand(2,4)];
end

%Object tracker parameter setting
P_G = 1;            %gating size in percentage
w_min = 0;           %hypothesis pruning threshold
merging_threshold = 2;  %hypothesis merging threshold
M = 100;                %maximum number of hypotheses kept
r_min = 1e-30;           %Bernoulli component pruning threshold
r_recycle = 0;        %Bernoulli component recycling threshold
...
%     PMBM = PMBM_ref;
    ...
    if k == 3
        assignin('base', 'ht3',PMBM.paras.MBM.ht);
        assignin('base', 'tt3_ref',PMBM_ref.paras.MBM.tt);
        assignin('base', 'tt3',PMBM.paras.MBM.tt);
    end
    %PMBM prediction
